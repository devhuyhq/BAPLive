...
export const createRoom = async (socket, data, ack) => {
    ...
    try {
        ...
        const admin = await BAPLive.database.collection(UserSchema.getSchemaName()).findOne({[UserSchema.getIdField()]: new ObjectID(socket.user[UserSchema.getIdField()])}, {[UserSchema.getPasswordField()]: 0});
...
export const joinRoom = async (socket, data, ack) => {
    ...
    if (userId) {
        ...
        response[MemberSchema.getDeviceIdField()] = deviceId;
    }

    sendAck(ack, dataResponse(response));
    socket.join(roomId, () => {
...
export const likeRoom = async (socket, data, ack) => {
    ...
    const user = await BAPLive.database.collection(UserSchema.getSchemaName()).findOne({[UserSchema.getIdField()]: new ObjectID(socket.user[UserSchema.getIdField()])}, {[UserSchema.getPasswordField()]: 0});

    BAPLive.io.sockets.emit(
        'on-like-room',
        dataResponse({user, roomId, likeCount})
    )
};
