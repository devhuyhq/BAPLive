...
export const createRoom = async (socket, data, ack) => {
    ...
    try {
        ...
        console.log(JSON.stringify(dataResponse(savedRoom)));
...
export const joinRoom = async (socket, data, ack) => {
    ...
    ack(dataResponse(updatedMember));
    console.log(JSON.stringify(dataResponse(updatedMember)));
    socket.join(roomId, () => {
...
export const changeStatus = async (socket, data, ack) => {
    ...
    if (status === 1) {
        updateData = {
            ...
            [RoomSchema.getUrlField()]: `rtmp://172.16.1.54:1935/live/${room._id}`,
        };
    ...
    ack(dataResponse(Object.assign({}, room, updateData)));

    console.log(dataResponse(Object.assign({}, room, updateData)));
...
export const likeRoom = async (socket, data, ack) => {
    ...
    console.log(JSON.stringify(dataResponse({roomId, likeCount})));
    BAPLive.io.sockets.emit(
        'on-like-room',
